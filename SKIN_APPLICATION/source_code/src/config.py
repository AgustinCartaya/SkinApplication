# @autor: Agustin CARTAYA

# this file contains all the global constants of the application

import os

# project location
PROJECT_FOLDER = os.sep.join(__file__.split(os.sep)[:-2])

# system folder separator
_S = os.sep

# language
ACTUAL_LANGUAGE = "en"

# data base
DB_PATH = PROJECT_FOLDER + _S + "db"
DB_NAME = "skin_application.sqlite"
DB_PATH_NAME = DB_PATH + _S + DB_NAME
DB_SCRIPT_PATH = DB_PATH + _S + "scripts"
DB_SCRIPT_NAME = "tables_creation.sql"
DB_SCRIPT_PATH_NAME = DB_SCRIPT_PATH + _S + DB_SCRIPT_NAME
TABLE_DOCTORS = "DOCTORS"
TABLE_PATIENTS = "PATIENTS"
TABLE_SKIN_LESIONS = "SKIN_LESIONS"
TABLE_VARIABLE_INPUTS = "VARIABLE_INPUTS"
COLUMN_DOCTORS_FIRST_NAME = "first_name"
COLUMN_DOCTORS_LAST_NAME = "last_name"
COLUMN_DOCTORS_PASSWORD = "password"

# assets
ASSETS_PATH = PROJECT_FOLDER + _S + "assets"
FILES_PATH = ASSETS_PATH + _S + "files"
IMAGES_PATH = ASSETS_PATH + _S + "images"
FILES_SKIN_LESION_IMAGES_PATH = FILES_PATH + _S + "skin_lesion_images"
FILES_MEDICAL_INFORMATION_PATH = FILES_PATH + _S + "medical_information"
FILES_SKIN_LESION_CHARACTERISTICS_PATH = FILES_PATH + _S + "skin_lesion_characteristics"

FILES_AI_SKIN_LESION_IMAGES_PATH = FILES_SKIN_LESION_IMAGES_PATH + _S + "ai"
FILES_AI_MEDICAL_INFORMATION_PATH = FILES_MEDICAL_INFORMATION_PATH + _S + "ai"
FILES_AI_SKIN_LESION_CHARACTERISTICS_PATH = FILES_SKIN_LESION_CHARACTERISTICS_PATH + _S + "ai"

FILES_CREATED_SKIN_LESION_IMAGES_PATH = FILES_SKIN_LESION_IMAGES_PATH + _S + "created"
FILES_CREATED_MEDICAL_INFORMATION_PATH = FILES_MEDICAL_INFORMATION_PATH + _S + "created"
FILES_CREATED_SKIN_LESION_CHARACTERISTICS_PATH = FILES_SKIN_LESION_CHARACTERISTICS_PATH + _S + "created"

# images
IMG_LOGO_NAME = "logo.png"
IMG_LOGO_PATH_NAME = IMAGES_PATH + _S + IMG_LOGO_NAME

IMG_TYPE_BODY2D = "body2d"
IMG_BODY2D_FRONT_NAME = "body2d_front.png"
IMG_BODY2D_FRONT_PATH_NAME = IMAGES_PATH + _S + IMG_BODY2D_FRONT_NAME

IMG_BODY2D_BACK_NAME = "body2d_back.png"
IMG_BODY2D_BACK_PATH_NAME = IMAGES_PATH + _S + IMG_BODY2D_BACK_NAME

IMG_BODY2D_LEFT_NAME = "body2d_left.png"
IMG_BODY2D_LEFT_PATH_NAME = IMAGES_PATH + _S + IMG_BODY2D_LEFT_NAME

IMG_BODY2D_RIGHT_NAME = "body2d_right.png"
IMG_BODY2D_RIGHT_PATH_NAME = IMAGES_PATH + _S + IMG_BODY2D_RIGHT_NAME

IMG_ACCEPTED_EXTESIONS = "png, jpg, jpeg, gif, tif, tiff, raw, svg, bmp"

# Artificial inteligence
AI_FOLDER_NAME = "ai"
AI_PATH = PROJECT_FOLDER + _S + AI_FOLDER_NAME
AI_RUN_FILE_NAME = "run"
AI_INFO_FOLDER_NAME = "info"
AI_REQUIREMENTS_FILE_NAME = "requirements"
AI_DESCRIPTION_FILE_NAME = "description"
AI_REQUIRED_IMAGES_JSON = "images"
AI_REQUIRED_MEDICAL_INFORMATION_JSON = "medical_information"
AI_REQUIRED_SKIN_LESION_CHARACTERISTICS_JSON = "skin_lesion_characteristics"

# patients data
PATIENTS_DATA_PATH = PROJECT_FOLDER + _S + "patients_data"
SKL_IMAGES_FOLDER_NAME = "images"
SKL_TIMELINE_FOLDER_NAME = "timeline"

# skin lession folder sufix
SKL_FOLDER_SUFIX = "skin_lesion_"

# views
ANCIENT_VIEW = "ANCIENT_VIEW"
CREATE_ACCOUNT_VIEW = "CREATE_ACCOUNT_VIEW"
LOGIN_VIEW = "LOGIN_VIEW"
PATIENTS_VIEW = "PATIENTS_VIEW"
UPSERT_PATIENT_VIEW = "UPSERT_PATIENT_VIEW"
UPSERT_PATIENT_MI_VIEW = "UPSERT_PATIENT_MI_VIEW"
UPSERT_PATIENT_PREVIEW_VIEW = "UPSERT_PATIENT_PREVIEW_VIEW"
UPSERT_SKIN_LESION_VIEW = "UPSERT_SKIN_LESION_VIEW"
CHECK_PATIENT_VIEW = "CHECK_PATIENT_VIEW"
AI_LAUNCHER_VIEW = "AI_LAUNCHER_VIEW"
IMAGES_VIEW = "IMAGES_VIEW"
AI_RESULTS_VIEW = "AI_RESULTS_VIEW"
TIMELINE_VIEW = "TIMELINE_VIEW"

# styles
STYLES_PATH = PROJECT_FOLDER + "/src/views/ui/styles"
GLOBAL_STYLES_NAME = "global.css"
GLOBAL_STYLES_PATH_NAME = STYLES_PATH + _S + GLOBAL_STYLES_NAME

# messages
MSG_ERROR = "MSG_ERROR"
MSG_WARNING = "MSG_WARNING"
